...
$uri = "https://graph.microsoft.com/beta/reports/credentialUserRegistrationDetails?`$filter=authMethods/any(t:t eq microsoft.graph.registrationAuthMethod'mobilePhone')"
#$uri = "https://graph.microsoft.com/beta/reports/credentialUserRegistrationDetails?`$filter=authMethods/any(t:t eq 'mobileSMS')"
...
$users | select id, userPrincipalName, userDisplayName -pv user | foreach {$i++
    ...
    Invoke-MgGraphRequest -Uri $uri -Method GET -OutputType PSObject | select -ExpandProperty value | select `
        ...
        smsSignInState
}  | export-csv .\sms_sign_in_report.csv -NoTypeInformation

...
function getAADuser{
    ...
    $uri = "https://graph.microsoft.com/beta/users?`$filter=userType eq 'Member' and AccountEnabled eq false and onPremisesSyncEnabled eq true&`$select=displayName,signInActivity,userPrincipalName,userType,onPremisesSyncEnabled,createdDateTime,accountEnabled,mail,lastPasswordChangeDateTime,onPremisesLastSyncDateTime"
    do{$results = $null
        for($i=0; $i -le 3; $i++){
            try{
...
getAADuser | select displayName,userPrincipalName,userType,accountEnabled,onPremisesSyncEnabled,Mail, `
    ...
    @{Name="onPremisesLastSyncDateTime";Expression={if($_.createdDateTime -ne $_.onPremisesLastSyncDateTime){(get-date $_.onPremisesLastSyncDateTime).tostring('yyyy-MM-dd')}}}, `
    ...
    @{Name="Domain";Expression={($_.mail -split("@"))[1]}} | where {$_.lastPasswordChangeDateTime -like "*" -and $_.lastPasswordChangeDateTime -ne $_.createdDateTime -and (New-TimeSpan -Start $_.onPremisesLastSyncDateTime -end $(get-date)).totaldays -gt 365}  | export-csv .\azuread_usernotsyncedinawhile.csv -notypeinformation
